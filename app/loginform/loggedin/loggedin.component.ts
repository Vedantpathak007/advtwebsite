...
onSessionstorage() {
  // Example data to store, replace these values with actual data
  const thirdkey = this.thirke ;
  const forthke = 'John Doe';
  const fifthke = 'profile-picture-url.jpg';
  const sixthke = 'Sample Sixth Key';


  // Store the data in sessionStorage
  sessionStorage.setItem('forthke', forthke);
  sessionStorage.setItem('fifthke', fifthke);
  sessionStorage.setItem('sixthke', sixthke);
  sessionStorage.setItem('thirke', thirdkey);
}
